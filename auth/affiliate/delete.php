...
try {
    ...
    try {
        ...
        if (!isset($user['aid']) && (!isset($user['role']) || $user['role'] !== 'admin')) {
            ...
            echo json_encode(['status' => false, 'message' => 'دسترسی غیرمجاز. فقط ادمین می‌تواند بازاریاب را حذف کند'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        error_log("JWT decode error in delete.php: " . $e->getMessage());
        http_response_code(401);
        echo json_encode(['status' => false, 'message' => 'احراز هویت ناموفق: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
        exit;
    }
} catch (Exception $e) {
    error_log("Auth error in delete.php: " . $e->getMessage());
    http_response_code(401);
...
try {
    ...
    echo json_encode([
        'status' => true,
        'message' => 'بازاریاب با موفقیت حذف شد'
    ], JSON_UNESCAPED_UNICODE);
} catch (PDOException $e) {
    if (isset($conn) && $conn->inTransaction()) {
        $conn->rollBack();
    }
    error_log("Affiliate delete error: " . $e->getMessage());
