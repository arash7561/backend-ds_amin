...
try {
    ...
    try {
        ...
        if (!isset($user['aid']) && (!isset($user['role']) || $user['role'] !== 'admin')) {
            ...
            echo json_encode(['status' => false, 'message' => 'دسترسی غیرمجاز. فقط ادمین می‌تواند بازاریاب را ویرایش کند'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        error_log("JWT decode error in update.php: " . $e->getMessage());
        http_response_code(401);
        echo json_encode(['status' => false, 'message' => 'احراز هویت ناموفق: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
        exit;
    }
} catch (Exception $e) {
    error_log("Auth error in update.php: " . $e->getMessage());
    http_response_code(401);
...
$affiliateId = isset($input['affiliate_id']) ? (int)$input['affiliate_id'] : 0;
$affiliateCode = isset($input['affiliate_code']) ? trim(strtoupper($input['affiliate_code'])) : '';

// بررسی ورودی‌های الزامی
if ($affiliateId <= 0) {
    http_response_code(400);
    echo json_encode(['status' => false, 'message' => 'شناسه بازاریاب نامعتبر است'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    // بررسی وجود بازاریاب
    $stmt = $conn->prepare("SELECT id, user_id, code FROM affiliates WHERE id = ?");
    ...
    // بررسی اینکه آیا این کد قبلاً توسط بازاریاب دیگری استفاده شده است
    $stmt = $conn->prepare("SELECT id FROM affiliates WHERE code = ? AND id != ?");
    $stmt->execute([$affiliateCode, $affiliateId]);
    if ($stmt->fetch()) {
        $conn->rollBack();
    ...
    // بروزرسانی کد بازاریاب
    $stmt = $conn->prepare("UPDATE affiliates SET code = ? WHERE id = ?");
    $stmt->execute([$affiliateCode, $affiliateId]);
    ...
    echo json_encode([
        'status' => true,
        'message' => 'کد بازاریاب با موفقیت بروزرسانی شد',
