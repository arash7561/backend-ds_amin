...
try {
    ...
    try {
        ...
        if (!isset($user['aid']) && (!isset($user['role']) || $user['role'] !== 'admin')) {
            ...
            echo json_encode(['status' => false, 'message' => 'دسترسی غیرمجاز. فقط ادمین می‌تواند درخواست‌ها را مشاهده کند'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        error_log("JWT decode error in requests.php: " . $e->getMessage());
        http_response_code(401);
        echo json_encode(['status' => false, 'message' => 'احراز هویت ناموفق: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
        exit;
    }
} catch (Exception $e) {
    error_log("Auth error in requests.php: " . $e->getMessage());
    http_response_code(401);
...
try {
    ...
    error_log("Affiliate requests error: " . $e->getMessage());
